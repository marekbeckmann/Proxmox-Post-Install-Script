...
if [[ $EUID -ne 0 ]]; then
    echo -e "${BRED}Root privileges are required to perform this operation${REG}" >&2
    exit 1
fi

hash sed 2>/dev/null || {
    echo -e "${BRED}sed is required but missing from your system${REG}" >&2
    exit 1
}

hash pveversion 2>/dev/null || {
    echo -e "${BRED}PVE installation required but missing from your system${REG}" >&2
    exit 1
}
...
if test -d "$OFFLINEDIR"; then
    ...
    hash curl 2>/dev/null || {
        echo -e "${BRED}cURL is required but missing from your system${REG}" >&2
        exit 1
    }
fi
...
if [ "$OFFLINE" = false ]; then
    curl -sSf -f https://raw.githubusercontent.com/ &>/dev/null || {
        echo -e "${BRED}Could not establish a connection to GitHub (https://raw.githubusercontent.com)${REG}" >&2
        exit 1
    }
